...
| `contact`     | string | no       | Organizer contact for zero-infrastructure replies ("reply to organizer"): a plausible email address or phone number, unverified. Encoders omit it when empty and MUST reject implausible values; decoders MUST stay lenient and treat an implausible value as absent when rendering. The contact is part of the payload and therefore visible to anyone who holds the URL. Added after v2 froze: it is an additive optional field, so it does not bump the version — v2 decoders that predate it ignore it (see Versioning rules). |
